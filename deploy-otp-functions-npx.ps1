# OTP Function'larını Deploy Et (npx kullanarak)
# Bu script npx ile Supabase CLI'yi kullanır, global kurulum gerektirmez

$env:SUPABASE_ACCESS_TOKEN="sbp_18e64fd74e0e7cd39423ab716355b6803da9b875"
$PROJECT_REF="xcvcplwimicylaxghiak"

Write-Host "🚀 OTP Function'larını Deploy Ediyor..." -ForegroundColor Cyan
...
Write-Host "  https://$PROJECT_REF.supabase.co/functions/v1/send-otp" -ForegroundColor White
Write-Host "  https://$PROJECT_REF.supabase.co/functions/v1/verify-otp" -ForegroundColor White
Write-Host ""

if ($failCount -eq 0) {
    Write-Host "🎉 Tüm OTP functions başarıyla deploy edildi!" -ForegroundColor Green
} else {
