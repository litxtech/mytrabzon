# OTP Functions Deploy Script
# .env dosyasını atlayarak deploy eder

Write-Host "OTP Functions Deploy Baslatiliyor..." -ForegroundColor Green
...
# Deploy
Write-Host "`nsend-otp function deploy ediliyor..." -ForegroundColor Cyan
supabase functions deploy send-otp --no-verify-jwt

if ($LASTEXITCODE -eq 0) {
    Write-Host "send-otp deploy basarili!" -ForegroundColor Green
} else {
    Write-Host "send-otp deploy basarisiz!" -ForegroundColor Red
}

Write-Host "`nverify-otp function deploy ediliyor..." -ForegroundColor Cyan
...
Write-Host "`nDeploy islemi tamamlandi!" -ForegroundColor Green
